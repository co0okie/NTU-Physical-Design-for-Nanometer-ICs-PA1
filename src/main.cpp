...
constexpr LogLevel LOG_LEVEL = TRACE;
...
int main(int argc, char* argv[]) {
    ...
    {
        size_t smaller_group_size = num_of_cells / 2;
        if ((1 - balance_degree) / 2 * num_of_cells > smaller_group_size) {
    ...
    LOG(DEBUG) << "\ngroup0_size: " << group0_size << ", group1_size: " << num_of_cells - group0_size << endl;
    ...
    for (size_t i = 0; i < num_of_cells; i++) {
        int64_t gain = 0;
        for (size_t j = 0; j < nets_of_cell[i].size(); j++) {
            const size_t net_index = nets_of_cell[i][j];
            if (group_of_cell[i] == 0) { // cell i at group 0, try to move it to group 1
                if (cell_count_in_group0_of_net[net_index] == 1) gain++;
                else if (cell_count_in_group0_of_net[net_index] == cells_of_net[net_index].size()) gain--;
            } else { // cell i at group 1, try to move it to group 0
                if (cell_count_in_group0_of_net[net_index] == 0) gain--;
                else if (cell_count_in_group0_of_net[net_index] == cells_of_net[net_index].size() - 1) gain++;
            }
        }
        cells_of_gain(gain).push_back(i);
        gain_of_cell[i] = gain;
        iterator_of_cell[i] = --cells_of_gain(gain).end();
        if (group_of_cell[i] == 0) {
    ...
    RUN(DEBUG) for (int64_t gain = max_gain(), cell_count = 0; cell_count < num_of_cells; gain--) {
        LOG(DEBUG) << "gain " << gain << ": ";
    ...
    size_t cut_size = get_cut_size(cells_of_net, cell_count_in_group0_of_net);
    LOG(DEBUG) << "cut_size: " << cut_size << endl;
    size_t min_cut_size = cut_size;
    ...
    for (size_t num_of_unlocked_cells = num_of_cells; num_of_unlocked_cells;) {
        ...
        LOG(DEBUG) << "pick " << name_of_cell[cell_to_move] << " group " << 
            (group_of_cell[cell_to_move] ? "1 -> 0" : "0 -> 1") << ": ";
        for (auto net : nets_of_cell[cell_to_move]) {
            LOG(DEBUG) << name_of_net[net] << "{";
            bool first = true;
            for (auto cell : cells_of_net[net]) {
        ...
        for (size_t i = 0; i < nets_of_cell[cell_to_move].size(); i++) {
            ...
            for (size_t j = 0; j < cells_of_net[net].size(); j++) {
                ...
                cells_of_gain(gain_of_cell[cell]).push_back(cell);
                iterator_of_cell[cell] = --cells_of_gain(gain_of_cell[cell]).end();
        ...
        if (group_of_cell[cell_to_move] == 0) {
            LOG(DEBUG) << name_of_cell[cell_to_move] << " 0 -> 1 gain " << max_gain() << " ";
            ...
            LOG(DEBUG) << name_of_cell[cell_to_move] << " 1 -> 0 gain " << max_gain() << " ";
        ...
        LOG(DEBUG) << "cut_size " << cut_size + gain_of_cell[cell_to_move] << " -> " << cut_size << endl;
        RUN(TRACE) for (int64_t gain = max_gain(), cell_count = 0; 
        cell_count < num_of_unlocked_cells; gain--) {
            LOG(TRACE) << "gain " << gain << ": ";
            if (cells_of_gain(gain).size()) 
                for (auto cell : cells_of_gain(gain)) { 
                    LOG(TRACE) << name_of_cell[cell] << " ";
    ...
    LOG(INFO) << "\nmin_cut_size = " << min_cut_size << " after " 
        << picked_No_after_min_cut << " cells picked" << endl;
    ...
    cut_size = get_cut_size(cells_of_net, cell_count_in_group0_of_net);
    LOG(INFO) << "cut_size after trace back: " << cut_size << endl;
    for (size_t cell = 0; cell < num_of_cells; cell++) {
        LOG(DEBUG) << name_of_cell[cell] << ":" << group_of_cell[cell] << " ";
    }
